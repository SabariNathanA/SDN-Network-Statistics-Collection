...
add_ipsoc = (\"INSERT INTO routeinfo (Sender_IP, Dest_IP, Sender_port, Dest_port, Switch_port) VALUES (%(sip)s,%(dip)s,%(sp)s,%(dp)s,%(s_p)s)\")\n\n\
while run:\n\
\ta = sniff(iface=[")
...
\t\t\t\tdata_ipsoc = {'sip': ip_src, 'dip': ip_dst, 'sp': tcp_sport,'dp': tcp_dport,'s_p':switch_port}\n\
...
opPy_file.write("s_p =  'openflow:'+str(switch_index)+':'+str(port_index)\n\tdata_ipsoc = {'s_p':s_p,'coll': coll, 'rb': rb, 'rcrc': rcrc,'rd':\n\trd,'rfe':rfe,'roe':roe,'rp':rp,'tb':tb,'td':td,'te':te,'tp':tp}\n\tcursor.execute(add_ipsoc,data_ipsoc)\n\tmysql_cnx.commit()\n\n")
...
# MySQL cursor -> aids insert\n\
cursor = mysql_cnx.cursor()\n\
add_ipsoc = (\"INSERT INTO stats (Swicth_port, Collisions,  RX_Bytes, RX_CRC_Errors, RX_Dropped, RX_Frame_Error, RX_Over_Error, RX_Packets, TX_Bytes, TX_Dropped, TX_Error, TX_Packets) VALUES (%(s_p)s,%(coll)s,%(rb)s,%(rcrc)s,%(rd)s,%(rfe)s,%(roe)s,%(rp)s,%(tb)s,%(td)s,%(te)s,%(tp)s)\")\n\n\
while run:\n")
